...
function handleOperator(nextOperator) {
  ...
  // if operator was just pressed and waiting for second operand
  if (pendingSecondOperator) {
    // updated the operator to the new one without calculating
  ...
  if (firstOperator === null) {
    ...
    if (result === "error") {
      displayValue = "forbidden ❌";
      updateDisplay();
      return;
    }

    const roundedResult = Number.parseFloat(result.toFixed(4)).toString();
...
function highlightActiveDigit(digit) {
  removeActiveDigit();
  digitButtons.forEach((button) => {
    if (button.dataset.digit === digit) {
      button.classList.add("active");
    }
  });
}

function removeActiveDigit() {
  digitButtons.forEach((button) => {
    button.classList.remove("active");
  });
}
